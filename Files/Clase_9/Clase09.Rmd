...
## Corrección Pública control 6

--

`r fa("info-circle", fill="red")` Hubo 5 estudiantes en 5.0. Y el Top 3 de la semana pasada entonces queda:
...
pop_df = 
  data.frame(
  lang = c("Bladimir Contreras", "Dilan Alarcon", "Juan Rapalino"),
  n_jobs = c(100, 95, 95),
  free = c(T, F, F)
  )
## Graficando
pop_df %>% 
  ...
  ylab("Podium") +
  labs(
    title = "Participación Control 06",
    subtitle = "Nota de Control 06"
    ) +
  scale_fill_manual(
    "Mejor tiempo?",
    labels = c("Si", "No"),
...
.pull-left[
```{R, graph02, echo = F, fig.height = 5, fig.width = 9, dev = "svg"}
# Datos
pop_df = 
  data.frame(
  lang = c("Miguel Romo", "Valeria Barrera", "Gonzalo Macias"),
  n_jobs = c(100, 95, 88),
  free = c(T, F, F)
  )
## Graficando
pop_df %>% 
  ...
  labs(
    title = "Control 07",
    subtitle = "Nota de control actual"
    ) +
  scale_fill_manual(
    "Mejor tiempo?",
    labels = c("Si", "No"),
    values = c("#f92672", "turquoise")
    ) +
  ggthemes::theme_pander(base_size = 17) +
  # theme_ipsum() +
  theme(legend.position = "bottom")
